/*
  # Fix RPC Function Column Types

  1. Changes
    - Cast phone to text to match varchar(20) actual type
    - Ensure all type mismatches are resolved
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(p.currency, 'EUR')::text as listing_currency,
    ...
    s.id as seller_id,
    COALESCE(s.full_name, s.name, 'Property Owner')::text as seller_name,
    s.avatar_url as seller_avatar_url,
    s.phone::text as seller_phone,
    s.email as seller_email,
    COALESCE(s.role, 'seller')::text as seller_role,
    COALESCE(s.plan_id, 'free')::text as seller_plan_id,
    COALESCE(s.verified, false) as seller_verified,
    COALESCE(p.property_type, 'Property')::text as property_type,
    COALESCE(p.available_from, 'Available Now')::text as available_from
  FROM properties p
