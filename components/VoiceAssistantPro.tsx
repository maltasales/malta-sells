    ...
    <div className={`max-w-2xl mx-auto bg-white rounded-2xl shadow-lg p-6 ${className}`}>
      ...
      <div className="space-y-4">
        <textarea
          ...
          placeholder="Ask Lucia about Malta properties... (e.g., 'Show me apartments in Sliema')"
          className="w-full px-4 py-3 border border-gray-300 rounded-lg resize-none focus:ring-2 focus:ring-[#D12C1D] focus:border-transparent"
          rows={3}
          maxLength={1000}
        ...
        <div className="flex justify-between items-center">
          <span className="text-xs text-gray-500">{textInput.length}/1000</span>
          <button
            onClick={sendTextToAPI}
            disabled={!textInput.trim() || recordingState === 'processing'}
            className={`px-6 py-2 rounded-lg font-medium transition-all duration-200 flex items-center space-x-2 ${
              !textInput.trim() || recordingState === 'processing'
                ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                : 'bg-[#D12C1D] text-white hover:bg-[#B8241A] active:scale-95'
            }`}
          >
