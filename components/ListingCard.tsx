...
import { Heart, Bed, Bath, MapPin, Calendar, ArrowUpRight, Palette } from 'lucide-react';
...
interface Listing {
  ...
  type: string;
  videoUrl?: string;
  furnishingStatus?: string; // "Furnished", "Unfurnished", "Shell form", etc.
}
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
      <div className="relative">
        ...
        {(listing.furnishingStatus === 'Unfurnished' || listing.furnishingStatus === 'Shell form' || 
          (!listing.furnishingStatus && (listing.id % 2 === 1))) && (
          <button
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
              window.open(`/ai-decoration?propertyId=${listing.id}&image=${encodeURIComponent(listing.image)}`, '_blank', 'width=1200,height=800');
            }}
            className="absolute top-3 left-3 p-2 rounded-lg hover:scale-110 transition-all duration-200"
          ...
          >
            <img 
              ...
              alt="AI Decoration"
              className="w-7 h-7 filter brightness-0 invert drop-shadow-lg"
        ...
        {listing.furnishingStatus && (
          <div className="absolute bottom-3 left-3 px-2 py-1 bg-black/70 backdrop-blur-sm rounded text-xs font-medium text-white">
            {listing.furnishingStatus}
          </div>
        )}
      </div>
