    ...
    try {
      ...
      if (syncError) {
        ...
        const fallbackVideos = properties?.map((property: any) => {
          ...
          console.log('🔍 CHECKING PROFILE for property:', property.id, 'seller_id:', property.seller_id);
          ...
          if (!sellerName && typeof window !== 'undefined') {
            ...
            if (currentUser) {
              ...
              if (user.id === property.seller_id) {
                ...
                console.log('🔄 USING LOCALSTORAGE NAME:', sellerName, 'for seller:', property.seller_id);
              }
            }
          }
          
          // If still no name, skip this property
          if (!sellerName) {
            console.error('❌ NO REAL NAME found for seller:', property.seller_id, '- SKIPPING this property');
            return null;
          }
          ...
          console.log('✅ USING REAL NAME:', sellerName, 'for property:', property.title);
        ...
        }).filter(Boolean) || []; // Filter out null values (properties without real names)
